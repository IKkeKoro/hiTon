...
async function Donation_init(deployer: Address, id: bigint) {
    ...
    initDonation_init_args({ $$type: 'Donation_init_args', deployer, id })(builder);
...
const Donation_getters: ABIGetter[] = [
    {"name":"donationData","arguments":[],"returnType":{"kind":"simple","type":"DonationData","optional":false}},
]

export const Donation_getterMapping: { [key: string]: string } = {
    'donationData': 'getDonationData',
}

const Donation_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"typed","type":"DeployDonation"}},
    {"receiver":"internal","message":{"kind":"typed","type":"ChangeActive"}},
    {"receiver":"internal","message":{"kind":"empty"}},
    {"receiver":"internal","message":{"kind":"typed","type":"Donate"}},
]

export class Donation implements Contract {
    
    static async init(deployer: Address, id: bigint) {
        return await Donation_init(deployer, id);
    }
    
    static async fromInit(deployer: Address, id: bigint) {
        const init = await Donation_init(deployer, id);
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: DeployDonation | ChangeActive | null | Donate) {
        
        let body: Cell | null = null;
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'DeployDonation') {
            body = beginCell().store(storeDeployDonation(message)).endCell();
        }
        ...
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'Donate') {
            body = beginCell().store(storeDonate(message)).endCell();
        }
    ...
    async getDonationData(provider: ContractProvider) {
        let builder = new TupleBuilder();
        let source = (await provider.get('donationData', builder.build())).stack;
        const result = loadGetterTupleDonationData(source);
        return result;
    }
